...
def build_features_from_panel(
    ...
    lag_seconds: tuple[float, ...] = (1.0, 2.0, 5.0),
    rolling_window_seconds: tuple[float, ...] = (3.0, 5.0, 10.0),
) -> pd.DataFrame:
    """
    Convert a fixed-time base panel into a feature table.

    This version is aligned to high-frequency panels like 100ms.
    Lag and rolling features are defined in real time (seconds), not raw row count.
    """
    df = panel_df.copy()
    ...
    feature_df["yes_depth_top3"] = _top_k_depth_vectorized(yes_arr, k=3)
    feature_df["yes_depth_top5"] = _top_k_depth_vectorized(yes_arr, k=5)

    feature_df["no_depth_top1"] = _top_k_depth_vectorized(no_arr, k=1)
    feature_df["no_depth_top3"] = _top_k_depth_vectorized(no_arr, k=3)
    feature_df["no_depth_top5"] = _top_k_depth_vectorized(no_arr, k=5)
    ...
    if include_imbalance:
        ...
        feature_df["imbalance_top3"] = _safe_imbalance_series(
            feature_df["yes_depth_top3"], feature_df["no_depth_top3"]
        )
        feature_df["imbalance_top5"] = _safe_imbalance_series(
            feature_df["yes_depth_top5"], feature_df["no_depth_top5"]
        )
    ...
    if add_recent_dynamics:
        ...
        for seconds in rolling_window_seconds:
            ...
            if include_imbalance:
                ...
                feature_df[f"imbalance_total_mean_{suffix}"] = (
                    feature_df["imbalance_total"].rolling(steps, min_periods=steps).mean()
                )
...
def build_all_feature_tables(
    ...
    for i, panel_path in enumerate(panel_files, start=1):
        ...
        # IMPORTANT:
        # Keep this filename aligned with train_grid.py, which searches *-features.parquet
...
if __name__ == "__main__":
    ...
    # lines 398-399
    parser.add_argument("--lag-seconds", default="1.0,2.0,5.0")
    parser.add_argument("--rolling-window-seconds", default="3.0,5.0,10.0") 
